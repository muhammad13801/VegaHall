...
export const getOwnerBookings = async (req, res) => {
    ...
    `;
        if (!bookings)
            return res.status(404).send("❌ لا توجد حجوزات");
...
export const proposeReschedule = async (req, res) => {
    ...
    `;
        ...
        await insertNotification(booking.customer_id, "طلب تغيير موعد الحجز", `اقترح صاحب صالة ${booking.hall_name} تغيير موعد حجزك إلى ${formattedDate}. يرجى القبول أو الرفض.`, "reschedule");
...
export const respondReschedule = async (req, res) => {
    ...
    `;
        ...
        WHERE id = ${id}
      `;
            await insertNotification(booking.owner_id, "تم قبول تعديل الموعد", `وافق العميل على تعديل موعد الحجز في صالة ${booking.hall_name}.`, "reschedule_accept");
        }
        else {
            await sql `
        UPDATE bookings
        ...
        WHERE id = ${id}
      `;
            await insertNotification(booking.owner_id, "تم رفض تعديل الموعد", `رفض العميل تعديل موعد الحجز في صالة ${booking.hall_name}. سيبقى الموعد الأصلي.`, "reschedule_reject");
        }
...
export const ownerCancelBooking = async (req, res) => {
    ...
    `;
      ...
      WHERE id = ${id}
    `;
        await insertNotification(booking.customer_id, "قام صاحب الصالة بإلغاء حجزك", `نأسف، قام صاحب صالة ${booking.hall_name} بإلغاء حجزك. سيتم استرجاع المبلغ تلقائياً.`, "cancel");
...
export const customerCancelBooking = async (req, res) => {
    ...
    `;
        ...
        await insertNotification(booking.owner_id, "طلب إلغاء حجز من العميل", `قام العميل بإلغاء حجزه في صالة ${booking.hall_name}. يرجى اتخاذ قرار بشأن استرجاع المبلغ.`, "customer_cancel");
...
export const customerCancelResponse = async (req, res) => {
    try {
        const { id } = req.params;
    ...
    `;
        ...
        await insertNotification(booking.customer_id, refund ? "تم استرجاع مبلغ حجزك" : "تم رفض استرجاع المبلغ", refund
            ? `قرر صاحب صالة ${booking.hall_name} استرجاع المبلغ المدفوع كاملاً.`
            : `قرر صاحب صالة ${booking.hall_name} عدم استرجاع المبلغ المدفوع.`, refund ? "refund_approved" : "refund_rejected");
