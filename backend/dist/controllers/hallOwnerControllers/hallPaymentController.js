...
export const charge = async (req, res) => {
    try {
        ...
        const ephemeralKey = await stripe.ephemeralKeys.create({ customer: customer.id }, { apiVersion: "2026-02-25.clover" });
...
export const confirmPayment = async (req, res) => {
    try {
        const userId = req.userId;
        const { paymentIntentId, name, capacity, price, city, address, latitude, longitude, description, images, videos, services, mealOptions, secondaryContacts, } = req.body;
        if (!userId)
            return res.status(401).send("❌ مستخدم غير مصرح");
        if (!paymentIntentId)
      ...
      `;
            ...
            if (mealOptions?.length) {
                await Promise.all(mealOptions.map(async (m) => {
                    const [mt] = await tx `
            ...
            if (images?.length) {
                await Promise.all(images.map((url) => tx `INSERT INTO media (hall_id, type, url) VALUES (${hallId}, 'image', ${url})`));
            }
            if (videos?.length) {
                await Promise.all(videos.map((url) => tx `INSERT INTO media (hall_id, type, url) VALUES (${hallId}, 'video', ${url})`));
            }
            ...
            if (secondaryContacts?.length) {
                await Promise.all(secondaryContacts.map((c) => tx `
