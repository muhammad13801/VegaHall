...
export const requestService = async (req, res) => {
    ...
    `;
        // Notify admins
        await Promise.all(admins.map((admin) => insertNotification(admin.id, "طلب خدمة جديد", `لقد طلب المستخدم ${ownerName} صاحب رقم: ${ownerId} خدمة جديدة هي: ${serviceName}`, "service_request")));
...
export const requestMeal = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).send("❌ مستخدم غير مصرح");
    ...
    `;
        // Notify admins
        await Promise.all(admins.map((admin) => insertNotification(admin.id, "طلب وجبة جديد", `لقد طلب المستخدم ${ownerName} صاحب رقم: ${ownerId} وجبة جديدة هي: ${mealName}`, "meal_request")));
        return res.send("✔️ تم إرسال طلبك للمراجعة");
    }
    catch (err) {
