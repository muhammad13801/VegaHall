...
import { checkPending, checkUserAvailability, generateCode, processVerification, } from "../../services/authService.js";
import { handleVerificationError } from "../../utils/handleVerificationError.js";
/* ================= REGISTER ================= */
export const register = async (req, res) => {
    try {
        const { firstName, lastName, gender, dateOfBirth, password, phoneNumber, role, } = req.body;
        ...
        if (availability.emailTaken) {
            ...
            if (availability.pending) {
                if (new Date(Date.now()) < availability.pending.expires_at &&
...
export const resendCode = async (req, res) => {
    try {
      ...
      WHERE email = ${email}
    `;
        await sendVerificationCode(result.user.email, result.user.first_name, result.user.last_name, newCode);
