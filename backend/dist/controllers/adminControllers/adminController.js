...
export const getPendingHalls = async (req, res) => {
    try {
...
export const approveHall = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id)
            return res.status(400).send("❌ معرف القاعة غير صالح");
        const [hall] = await sql `
      SELECT owner_id, hall_name FROM halls WHERE id = ${id}
    `;
        if (!hall)
            return res.status(404).send("❌ القاعة غير موجودة");
        await sql `
      UPDATE halls 
      SET status = 'active'
      WHERE id = ${id}
    `;
        await insertNotification(hall.owner_id, "تهانينا! تم تفعيل قاعتك", `تمت الموافقة على انضمام صالتك (${hall.hall_name}) إلى النظام بنجاح.`, "hall_approved");
...
export const addServiceToHall = async (req, res) => {
    try {
    ...
    `.then((res) => res[0]);
      ...
      VALUES (${hallId}, ${service.id}, ${price})
      ON CONFLICT (hall_id, service_id) DO UPDATE SET price = EXCLUDED.price
    `;
        await insertNotification(hall.owner_id, "خدمة جديدة لقاعتك", `تم إضافة/تحديث الخدمة (${name}) في صالتك: ${hall.hall_name}`, "service_added");
...
export const addServiceToAllHalls = async (req, res) => {
    try {
        ...
        await sql.begin(async (tx) => {
            ...
            let service = await tx `SELECT id FROM services WHERE name = ${name}`.then((res) => res[0]);
            if (!service) {
            ...
            for (const hall of halls) {
                await tx `
          INSERT INTO hall_services (hall_id, service_id, price)
          VALUES (${hall.id}, ${service.id}, ${price})
          ON CONFLICT (hall_id, service_id) DO UPDATE SET price = EXCLUDED.price
        `;
                await insertNotification(hall.owner_id, "خدمة جديدة لقاعتك", `تم إضافة الخدمة العامة (${name}) في صالتك: ${hall.hall_name}`, "service_added");
            }
        });
...
export const getAllHallsSimple = async (req, res) => {
    try {
        const halls = await sql `
...
/* ================= SERVICE MANAGEMENT ================= */
export const getGlobalServices = async (req, res) => {
    try {
...
export const approveServiceRequest = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id)
            return res.status(400).send("❌ معرف الطلب غير صالح");
    ...
    `;
      ...
      `;
            // 3. Notify owner
            await insertNotification(request.owner_id, "تمت الموافقة على طلب الخدمة", `تم قبول طلبك لإضافة خدمة: ${request.name}. يمكنك الآن إضافتها لقاعتك.`, "service_approved");
        });
...
export const rejectServiceRequest = async (req, res) => {
    ...
    `;
        await insertNotification(request.owner_id, "تم رفض طلب الخدمة", `نعتذر، لقد تم رفض طلبك لإضافة خدمة: ${request.name}.`, "service_rejected");
...
export const addGlobalService = async (req, res) => {
    try {
        const { name } = req.body;
        if (!name)
            return res.status(400).send("❌ اسم الخدمة مطلوب");
        await sql `
      INSERT INTO services (name)
      VALUES (${name})
      ON CONFLICT (name) DO NOTHING
...
/* ================= MEAL MANAGEMENT ================= */
export const getGlobalMealTypes = async (req, res) => {
    try {
...
export const getMealRequests = async (req, res) => {
    try {
        const requests = await sql `
...
export const approveMealRequest = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id)
            return res.status(400).send("❌ معرف الطلب غير صالح");
    ...
    `;
      ...
      `;
        ...
        ON CONFLICT (name) DO NOTHING
      `;
            await insertNotification(request.owner_id, "تمت الموافقة على طلب الوجبة", `تم قبول طلبك لإضافة نوع وجبة: ${request.name}. يمكنك الآن إضافتها لقاعتك.`, "meal_approved");
        });
...
export const rejectMealRequest = async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id)
            return res.status(400).send("❌ معرف الطلب غير صالح");
    ...
    `;
        await insertNotification(request.owner_id, "تم رفض طلب الوجبة", `نعتذر، لقد تم رفض طلبك لإضافة نوع وجبة: ${request.name}.`, "meal_rejected");
        res.send("✔️ تم رفض الطلب");
    }
    catch (err) {
        console.error(err);
...
export const addGlobalMealType = async (req, res) => {
    try {
        const { name } = req.body;
        if (!name)
            return res.status(400).send("❌ اسم الوجبة مطلوب");
        await sql `
      INSERT INTO meal_types (name)
      VALUES (${name})
      ON CONFLICT (name) DO NOTHING
