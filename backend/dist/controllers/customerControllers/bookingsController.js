...
export const cancelBooking = async (req, res) => {
    ...
    `;
        ...
        await insertNotification(booking.owner_id, "إلغاء حجز", `قام العميل بإلغاء حجز في صالة ${booking.hall_name}. يرجى مراجعة إدارة الحجوزات.`, "customer_cancel");
...
export const requestReschedule = async (req, res) => {
    ...
    if (!proposed_date)
        return res.status(400).send("❌ تاريخ مقترح مفقود");
    try {
        const [booking] = await sql `
    ...
    `;
        await insertNotification(booking.owner_id, "تعديل موعد حجز", `قام ${booking.first_name} ${booking.last_name} بتغيير موعد حجز قاعتك من تاريخ ${oldDateFormatted} الى الموعد الجديد وهو: ${newDateFormatted} للمزيد من التفاصيل الرجاء الاطلاع على "ادارة الحجوزات"`, "reschedule_request");
